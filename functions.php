...
add_action( 'wp_enqueue_scripts', 'aibc_enqueue_styles', PHP_INT_MAX);
function aibc_enqueue_styles() {
    ...
    wp_enqueue_style('aibc-responsive', CHILD_DIR . '/assets/css/responsive.css');
    wp_enqueue_style('aibc-slick', CHILD_DIR . '/assets/css/slick.css');
    wp_enqueue_style('aibc-slick-theme', CHILD_DIR . '/assets/css/slick-theme.css');
    wp_enqueue_style('aibc-slick-lightbox', CHILD_DIR . '/assets/css/slick-lightbox.css');
    wp_enqueue_style( $parent_style, get_template_directory_uri() . '/style.css' );
}

// load js files in footer & style in header start
add_action('wp_enqueue_scripts', 'aibc_scripts');
function aibc_scripts() {
    ...
    wp_enqueue_script('aibc-slick-script', CHILD_DIR . '/assets/js/slick.min.js', array(), '1.0.0', true );
    wp_enqueue_script('aibc-slick-lightbox-script', CHILD_DIR . '/assets/js/slick-lightbox.js', array(), '1.0.0', true );
...
add_shortcode('aibc-wpbsearch', 'aibc_wpbsearchform');
function aibc_wpbsearchform( $form ) {
...
add_action('wp_ajax_nopriv_autocompleteSearch', 'aibc_autocomplete_search');
add_action('wp_ajax_autocompleteSearch', 'aibc_autocomplete_search');
function aibc_autocomplete_search() {
...
function aibc_get_news_tags_lists() {
    $taxonomies = array( 
        'news-tag',
    );
    $args = array(
...
function aibc_get_magazines($term_id) {
...
add_shortcode( 'aibc-banner-adds', 'aibc_banner_adds' );
function aibc_banner_adds($atts) {
...
add_shortcode( 'aibc-newsletter', 'aibc_newsletter' );
function aibc_newsletter() {
    $output = '';
    ...
    $output .='<div class="newsletter" style="background: '.$bg_image.'">
                <div class="container">
...
function aibc_get_video_term($page_id) {
    $taxonomy = 'videos-cat';
...
function aibc_get_people_list($atts) {
    $content = '';
    $taxonomy = 'people-cat';
    ...
    if(!empty($get_posts)) {
        foreach($get_posts as $k => $post) {
            ...
            $people_icon = ($person_image != NULL && $person_image == 'YES' ? (wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'full' )) : []);
            ...
            if($appearance === $appearanceRegular) {
				...
				$content .= '>';
                    if(!empty($people_icon)) { $content .= '<img src="'. $people_icon[0] .'" alt="">'; }
                ...
                $content .= '<div id="" class="single-sponsors-exhibitors'.$post->ID.' item '.$fullclass.'">';
                                ...
                                $content .= '<div class="left">';
                                    ...
                                    if(!empty($people_icon)) { 
                                            $content .= '<div class="avatar" style="background-image: url(' . "'" .$people_icon[0]."'" . ')"></div>';
                                    }
								$content .= 
...
function load_people_by_ajax_callback() {
    ...
    if(!empty($get_posts)) {
        foreach($get_posts as $k => $post) {
            $title = $post->post_title;
            $people_icon = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'full' );
            $people_designation = get_field('designation', $post->ID);
            $people_company = get_field('company', $post->ID);
            $content .= '<div class="single-speaker">';
                if($person_image === 'YES' && !empty($people_icon)) { $content .= '<img src="'. $people_icon[0] .'" alt="">'; }
...
function aibc_get_videos($term_id, $posts_per_page, $appearance = '') {
    $taxonomy = 'videos-cat';
...
add_shortcode( 'aibc-about-videos', 'aibc_get_about_videos' );
function aibc_get_about_videos($atts) {
    $term_id       = isset($atts['term_id']) ? $atts['term_id'] : '';
    $posts_per_page = isset($atts['posts_per_page']) ? $atts['posts_per_page'] : '-1';
    $appearance = isset($atts['appearance']) ? $atts['appearance'] : '';
    $content = '';
    ...
    $posts_by_year = [];
    $get_videos = aibc_get_videos($term_id, $posts_per_page, $appearance);
    if(!empty($get_videos)) {
        if($appearance === 'SIGMATV') {
...
function aibc_get_company_term($page_id) {
    $supported_by = array();
...
add_shortcode( 'aibc-company-lists', 'aibc_get_company' );
function aibc_get_company($atts) {
    $content = '';
    $taxonomy = 'company-cat';
...
//Shortcode to get company.
add_shortcode( 'aibc-single-company-row', 'aibc_get_single_company_row' );
function aibc_get_single_company_row($atts) {
...
function aibc_get_sponsoring_tags_data($tag_id, $taxonomy, $count) {
    $tag_data = array();
    $post_data = array();
    $tag_category = get_term_by('id', $tag_id, $taxonomy);
    if(isset($tag_id) && !empty($tag_id)) {
...
add_shortcode( 'aibc-available-sold-text', 'aibc_get_available_sold_text' );
function aibc_get_available_sold_text() {
    $content = '';
...
add_shortcode( 'aibc-sponsors-accordian-tabs', 'aibc_get_sponsors_accordian_tabs_data' );
function aibc_get_sponsors_accordian_tabs_data($atts) {
    $content = '';
    $tag_data = array();
    $post_data = array();
    ...
    $get_posts = get_posts($post_tag_args);
    $aval_sold = do_shortcode( '[aibc-available-sold-text]' );
...
add_shortcode( 'aibc-book-flight-form', 'aibc_book_flight_form' );
function aibc_book_flight_form() {
    $content = '';
...
add_shortcode( 'aibc-get-hotels', 'aibc_get_hotels' );
function aibc_get_hotels($atts) {
    $content = '';
    $taxonomy = 'hotel-cat';
    $term_id = isset($atts['term_id']) ? $atts['term_id'] : '';
    $count = isset($atts['post_per_page']) ? $atts['post_per_page'] : -1;
...
add_shortcode( 'aibc-get-awards', 'aibc_get_awards' );
function aibc_get_awards($atts) {
    $content = '';
    $term_id = $atts['term_id'];
    $count = isset($atts['post_per_page']) ? $atts['post_per_page'] : -1;
...
add_shortcode( 'aibc-igaming-gallery', 'aibc_igaming_gallery' );
function aibc_igaming_gallery($atts) {
    global $wp_query;
    $content = '';
    $posts_by_year = [];
    $count = isset($atts['post_per_page']) ? $atts['post_per_page'] : -1;
...
add_shortcode( 'aibc-seating-arrangments', 'aibc_seating_arrangments' );
function aibc_seating_arrangments($atts) {
    $content = '';
...
add_shortcode( 'aibc-related-articles', 'aibc_related_articles' );
function aibc_related_articles($atts) {
    $content = '';
    $count = isset($atts['post_per_page']) ? $atts['post_per_page'] : -1;
...
function aibc_get_all_company_lists_array($term_id, $count) {
    $taxonomy = 'company-cat';
    $post_type = 'company-items';
    if(!empty($term_id)) {
        $post_args = array(
...
add_shortcode( 'aibc-logos', 'aibc_get_logos' );
function aibc_get_logos($atts) {
    $content = '';
    $term_id = isset($atts['term_id']) ? $atts['term_id'] : '';
    $count = isset($atts['post_per_page']) ? $atts['post_per_page'] : -1;
    $appearance = isset($atts['appearance']) ? $atts['appearance'] : '';
    $colorClass = isset($atts['color']) ? $atts['color'] : '';
    $companyLists =  aibc_get_all_company_lists_array($term_id, $count);
...
add_shortcode( 'aibc-pitch-logo-slider', 'aibc_pitch_logo_slider' );
function aibc_pitch_logo_slider() {
    $content = '';
...
add_shortcode( 'aibc-last-year-startups-filter', 'aibc_last_year_startups_filter' );
function aibc_last_year_startups_filter($atts) {
    $content = '';
    $colorClass = isset($atts['color']) ? $atts['color'] : '';
...
//Shortcode to display banner adds
add_shortcode( 'aibc-magazines', 'aibc_magazines' );
function aibc_magazines($atts) {
    $content = '';
...
// For upcoming news
add_shortcode('upcoming-event', 'aibc_upcoming_event_shortcode');
function aibc_upcoming_event_shortcode(){
...
add_shortcode( 'aibc_show_sidebar_event', 'aibc_show_sidebar_event' );
function aibc_show_sidebar_event($atts) {
...
add_shortcode( 'aibc_show_sidebar_casinos', 'aibc_show_sidebar_casinos' );
function aibc_show_sidebar_casinos($atts) {
    $content = '';
    $term_id = '1378';
    $count = -1;
    $results =  aibc_get_casino_provider_data($term_id, $count);
    if(!empty($results)) {
...
add_shortcode( 'aibc_show_sidebar', 'aibc_show_sidebar' );
function aibc_show_sidebar($atts) {
...
add_shortcode( 'aibc-calendar-subentries', 'aibc_calendar_subentries' );
function aibc_calendar_subentries($atts) { 
...
add_shortcode( 'aibc-calendar-buttons', 'aibc_calendar_buttons' );
function aibc_calendar_buttons($atts) {
...
// Shortcode for calendar entries (day-long events)
add_shortcode( 'aibc-calendar-entries', 'aibc_calendar_entries' );
function aibc_calendar_entries($atts) {
...
// Shortcode for calendar entries (day-long events)
add_shortcode( 'aibc_latest_news', 'aibc_latest_news' );
function aibc_latest_news($atts) {
...
//shortcode to get Awards
add_shortcode( 'aibc-deep-tech-insights', 'aibc_deep_tech_insights' );
function aibc_deep_tech_insights($atts) {
    $content = '';
    $term_id = $atts['term_id'];
    $count = isset($atts['post_per_page']) ? $atts['post_per_page'] : -1;
    $post_tag_args = array(
